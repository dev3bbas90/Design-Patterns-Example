...
<div class="card card-custom">
    <div class="card-header py-3">
        <div class="card-title">
            ...
            <h3 class="card-label">{{__('Events')}}</h3>
        </div>
        ...
        <table class="table  table-head-custom table-checkable" id="kt_datatable1">
            ...
            <tbody>
               @foreach($events as $event)
                <tr>
                    <td>{{$event->id}}</td>
                    <td>{{$event->hall->theater->name}}</td>
                    <td>{{$event->hall->code}}</td>
                    <td>{{$event->program->title}}</td>
                    <td>{{$event->date}}</td>
                    <td>From : {{$event->time_frame->from}} <br> To : {{$event->time_frame->to}}</td>
                    <td>
                        <a href="{{route('booking.index',['event_id'=>$event->id])}}" class="btn btn-success" style=" width: 112px; ">{{__('Booking')}}</a>
                    </td>
                    <td>
                        <a href="{{route('events.edit',$event)}}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
